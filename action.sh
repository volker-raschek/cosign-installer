...
if ! command -v envsubst >/dev/null; then
  echo "envsubst command not found. Try to detect OS and distribution to provide installation instructions for envsubst."
    ...
    if [ -f /etc/os-release ]; then
      ...
      case "$ID" in
        ...
        fedora|rhel|centos)
          ...
          ;;
        *)
          echo "Please refer to your distribution's documentation for installing envsubst."
          ;;
      esac
    else
      echo "Unable to detect Linux distribution. Please refer to your distribution's documentation for installing envsubst." 1>&2
      exit 1
    fi
  else
    echo "Unsupported OS type: $OSTYPE. Please refer to your system's documentation for installing envsubst." 1>&2
    exit 1
  fi
